...
const Images = () => {
    ...
    const fetchMoreMount = async () => {
        ...
        if (dataState.length == 0) {
            const requestData = await getItems(`http://localhost:8000/routes/files/getimages?skip=${page}&limit=${5}`);
                    ...
                    return (
                        <div key={index} className={`feed-item border ${index <= 0 ? 'mt-0' : 'mt-28'} shadow-xl border-gray-400 rounded bg-white`}>
                            ...
                            <div className="feed-img flex items-center justify-center">
                                {elem.category == 'image' ? <img onDoubleClick={() => { addLikes(elem._id) }} className='max-h-[44rem] w-full' src={elem.fileType} alt="" /> : <video className='object-top max-h-[44rem] w-full outline-none object-cover' controls src={elem.fileType}></video>}
                            </div>
